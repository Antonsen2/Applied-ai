...
def test_api_classify_image():
    ...
    response = client.post("api/classify", files=[("images", image1), ("images", image2)])
    ...
    test_result = {"result": wildfire_control.run_model(processed_image1), "filename": "upload", "coords": None}, {"result": wildfire_control.run_model(processed_image2), "filename": "upload","coords": None}
...
def test_classify_image():
    ...
    with open('test_data/forest-1.jpg', 'rb') as f:
        image2 = f.read()
    response = client.post("/classify", files=[("images", image1), ("images", image2)])
    assert response.status_code == 200
